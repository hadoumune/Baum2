...
(function() {
  ...
  Baum = (function() {
    ...
    Baum.prototype.runOneFile = function(after_close) {
      var copiedDoc, folderSuccess, useArtboard;
      this.folderName = activeDocument.name.split(".psd").join('');
      this.folderPath = activeDocument.path.fullName + '/Output/' + this.folderName;
      this.saveFolder = new Folder(this.folderPath);
      folderSuccess = this.saveFolder.create();
      if (!folderSuccess) {
      ...
      this.root = copiedDoc;
      Util.deselectLayers();
      this.parseConfigLayers(copiedDoc);
    ...
    Baum.prototype.parseConfigLayers = function(root) {
    ...
    Baum.prototype.artboardPos = function() {
      var b, desc364, desc365, desc366, idLyr, idOrdn, idTrgt, idartboard, idartboardRect, ideditArtboardEvent, idnull, l, r, rect, ref23, t;
    ...
    Baum.prototype.artboardToGroup = function(document, artboardLayer) {
      var layerName, pos;
      layerName = artboardLayer.name.slice(8);
      document.activeLayer = artboardLayer;
      pos = this.artboardPos();
      (function(layerName) {
        var desc231, idLyr, idOrdn, idTrgt, idnull, idungroupLayersEvent, ref3;
        ...
        return executeAction(idungroupLayersEvent, desc231, DialogModes.NO);
      })(layerName);
      (function(layerName) {
        var desc236, desc237, idFrom, idLyr, idMk, idNm, idOrdn, idTrgt, idUsng, idlayerSection, idlayerSectionEnd, idlayerSectionStart, idnull, ref4, ref5;
        ...
        return executeAction(idMk, desc236, DialogModes.NO);
      })(layerName);
      return pos;
    };

    Baum.prototype.convertArtboard = function(document) {
      var artboardLayerList, findArtboard, i, j, k, layer, len, len1, len2, m, pos, posList, ref1, ref2;
      findArtboard = false;
      artboardLayerList = [];
      posList = [];
      ...
      if (findArtboard) {
        for (k = 0, len1 = artboardLayerList.length; k < len1; k++) {
          layer = artboardLayerList[k];
          pos = this.artboardToGroup(document, layer);
          posList.push(pos);
        }
        ref2 = document.layers;
    ...
    Baum.prototype.psdToJson = function(targetDocument, docName, useArtboard) {
      var json, toJson;
      toJson = new PsdToJson();
      json = toJson.run(targetDocument, docName, useArtboard);
      return Util.saveText(this.saveFolder + "/" + docName + ".layout.json", json);
    };

    Baum.prototype.psdToImage = function(targetDocument, docName) {
      var json, toImage;
      toImage = new PsdToImage();
      return json = toImage.run(targetDocument, this.saveFolder, docName);
    };
  ...
  PsdToJson = (function() {
    ...
    PsdToJson.prototype.run = function(document, documentName, useArtboard) {
      var bounds, canvasBase, canvasLayer, canvasSize, imageSize, json, layers;
      layers = this.allLayers1st(document, document, useArtboard);
